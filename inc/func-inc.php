...
function createTask($cnn, $tName, $tDesc, $tprog, $priority, $due){
    $sql = "INSERT INTO tasks (tName, tDesc, tprog, priority, due) VALUES(?,?,?,?,?);";

    $stmt = mysqli_stmt_init($cnn);
    if(!mysqli_stmt_prepare($stmt, $sql)){
        header("location: ../nav/task.php?error=stmtfailed");
        exit();
    }

    mysqli_stmt_bind_param($stmt, "sssss", $tName, $tDesc, $tprog, $priority, $due);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("location: ../nav/task.php?taskcreated");
        exit();
}

function createSask($cnn,  $tDesc, $tprog, $priority, $start, $due){
    $sql = "INSERT INTO tasks ( tDesc, priority, sdate, ddate) VALUES(?,?,?,?);";
